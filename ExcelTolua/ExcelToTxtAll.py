...
def excel_table_byname(fileName,SheetName ,index,nameindx,openStatus="w"):
    data = open_excel(path)
    table = data.sheet_by_name(SheetName)
    nrows = table.nrows #行数 
    colnames = table.row_values(0);
...
def main():
  ...
  insertServerList(SheetName="ios官方")

  excel_table_byname(SheetName="混服",fileName="jinshanyunAll",index=1,nameindx=0,openStatus = "w");
  excel_table_byname(SheetName="应用宝",fileName="tenxunyunAll",index=1,nameindx=3,openStatus = "w");
  excel_table_byname(SheetName="ios官方",fileName="aliyunAll",index=1,nameindx=0,openStatus = "w");
